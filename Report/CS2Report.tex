...
Correspondingly, the phase transitions smoothly from $90^\circ$ at negative frequencies, through $0^\circ$ at DC, down to $-90^\circ$ at positive frequencies. 
However, because the numerical simulation relies on a discrete approximation (using the fine time step $dt_{\text{fine}}$), a slight difference is observable between the numerical phase and the theoretical LTI continuous-time phase.
...
Figure \ref{fig:task1c} illustrates the digitized output $y[n]$ across fast, medium, and slow detector IRFs. 
...
To account for this in the estimation process, the system's IRF $h_d(t)$ must be isolated and measured experimentally
(for example, by using a purely scattering sample with $\tau \approx 0$). Once $h_d(t)$ is known, standard direct fitting is abandoned in favor of \textit{iterative reconvolution} \cite{Datta2020}.
In this process, a theoretical exponential decay is mathematically convolved with the measured detector IRF, and the resulting synthetic curve is compared to the measured data $y[n]$.
The parameter $\tau$ is iteratively adjusted until the synthetic curve matches the data, allowing the true fluorescence lifetime to be decoupled from the hardware-induced temporal broadening \cite{Datta2020}.
